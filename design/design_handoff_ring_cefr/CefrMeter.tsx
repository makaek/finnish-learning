/**
 * CefrMeter.tsx — the dashboard CEFR level meter, REDESIGN (Mock A, green).
 *
 * Replaces the old "A1.1 [bar] 1/3 уровней · 37%" strip. Mirrors the dashboard
 * hero ("Текущая ступень") and extends it with a 12-cell band rail — one cell
 * per LEVEL, grouped into the four CEFR bands — so both "where am I in this
 * band" and "how far to A2" answer at a glance.
 *
 *   • level chip (gold) · «Текущая ступень» · big band id (teal) + band name
 *   • right: % through the current band · "до <nextBand>"
 ...
 *
 * Accent is the app GREEN/teal (#1B8E84); the level chip stays gold (#C8902B)
 * to match the existing hero.
 */

const COL = {
  ink: "#23262E", sub: "#71757F", faint: "#AAB1BC", card: "#FFFFFF",
  line: "#E7E3DB", track: "#EEEAE2", teal: "#1B8E84", gold: "#C8902B", goldSoft: "#F6EEDC",
};
...
export interface CefrState {
  ...
  pct: number;           // 0..1 progress through the current band
