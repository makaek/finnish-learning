/**
 * grammar.integrity.test.ts — invariants over the REAL data/grammar.seed.json.
 *
 * Checks the authoring contract between the finnish-linguist's seed and the runtime
 * (parser + grader), per the invariants the linguist flagged when authoring slice 1:
 * markup balance, plain accepted/wrong strings, option/answer consistency, the
 * accepted∩wrong collision trap, and topic-graph integrity (prereqs/modules/rules resolve,
 * no cycles). A failure here means the CONTENT is malformed, not the code.
 */
...
const content = flattenGrammar(seed as RawGrammarFile);
const ruleIds = new Set(flattenRules(ruleSeed as RawRulesFile).map((r) => r.id));

/** Raw item/topic counts straight from the JSON, to prove the parser dropped nothing. */
const rawTopics = (seed as { topics?: unknown[] }).topics ?? [];
const rawItems = (seed as { items?: unknown[] }).items ?? [];
...
describe("grammar seed: shape", () => {
  it("parses every authored topic and item (nothing silently dropped)", () => {
    expect(content.topics.length).toBe(rawTopics.length);
    expect(content.items.length).toBe(rawItems.length);
    expect(content.topics.length).toBeGreaterThanOrEqual(7);
...
describe("grammar seed: topic graph", () => {
  const ids = new Set(content.topics.map((t) => t.id));
...
describe("grammar seed: items", () => {
  it("keeps accepted/wrong answers plain (no markup characters)", () => {
    for (const i of content.items) {
      if (i.type === "produce_form" || i.type === "transform") {
        for (const a of i.accepted) expect(MARKUP.test(a), `${i.id}: markup in accepted "${a}"`).toBe(false);
        for (const w of i.wrong) expect(MARKUP.test(w.match), `${i.id}: markup in wrong "${w.match}"`).toBe(false);
      }
      if (i.type === "fill_table") {
        for (const c of i.cells)
          for (const a of c.accepted) expect(MARKUP.test(a), `${i.id}: markup in cell accepted "${a}"`).toBe(false);
      }
    }
  });
  ...
  it("never lists a predicted-wrong form that is also accepted", () => {
    for (const i of content.items) {
      ...
      for (const w of i.wrong)
        expect(acc.has(normalizeFi(w.match)), `${i.id}: wrong "${w.match}" is also accepted`).toBe(false);
    }
  });
  ...
  it("gives every fill_table all six persons or a coherent row set", () => {
    for (const i of content.items) {
